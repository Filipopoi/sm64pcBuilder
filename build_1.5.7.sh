...
OLD_BUILD_SH=./build_1.5.6.sh
NEXT_UPDATE=./build_1.5.8.sh
...
if [ -f "$OLD_BUILD_SH" ]; then
    ...
    This is the folder where your build.sh files will generate in,                  
    as well as your sm64pc-master or sm64pc-nightly folders.
    You can delete any build.sh file that is outside of
    the sm64pcBuilder.

    Your old sm64pc-master or sm64pc-nightly folders are in
    the same location as they were. Either move them to the
    sm64pcBuilder folder, or select yes to updating when the
    message appears below to generate new folders.

    ${RESET}${YELLOW}------------------------------${RESET}
    ${CYAN}build.sh v1.5.7${RESET}
    ${YELLOW}==============================${RESET}
EOF
	sleep 10
	cd ./sm64pcBuilder
...
EOF
    ...
    "2")  if [[ -f "./enhancements/60fps_interpolation_wip_nocap.patch" ]]; then
            ...
            ;;
    "3")  wget https://cdn.discordapp.com/attachments/710283360794181633/717153070768717944/HD_Mario_Model.rar
